  ...
  let make = (~id, _children) => {
    ...
    render: _self =>
      <Queries.DeleteProjectMutation>
        ...{
             (mutate, _) => {
               let mutation = Queries.DeleteProject.make(~id, ());
               <MutationButton
                 mutate={
                   mutate(
  ...
  let make = (~project, _children) => {
    ...
    render: _self =>
      <div>
        ...
        <div> <DeleteProjectButton id=project##id /> </div>
      </div>,
  };
};
                 ...
                 | Data(response) =>
                   <div>
                     ...{
                          Array.map(
                            project =>
                              <div> <ProjectItem project /> <hr /> </div>,
                            response##projects,
                          )
                        }
                   </div>
        ...
        ...{
             (mutate, {result}) => {
               let resultFromResponse = response =>
                 Queries.AddProjectMutation.convertJsInputToReason(response).
                   result;
...
let make = _children => {
  ...component,
  render: _self => <div> <ProjectsList /> <NewProjectForm /> </div>,
};
