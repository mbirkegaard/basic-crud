...
let make = (
  ...
  render: self => {
    ...
    <input 
      ...
      onChange=(evt => ReactEvent.Form.target(evt)##value -> UpdateText |> self.send)
      onKeyDown=(evt => ReactEvent.Keyboard.key(evt) == "Enter" ? handleMutation() : ()) 
    />
    
    <button disabled=(self.state.text == "") onClick=((_e) => handleMutation())>
