...
[@bs.module "auth0-js"] [@bs.new] external createClient : clientOptions => auth0Client = "WebAuth";
...
let jscbToResultcb : (Result.t('a, string) => unit) => ((Js.null(string), Js.null('a)) => unit) = (callback) =>
  (err, res) => switch (Js.nullToOption(err), Js.nullToOption(res)) {
...
let userInfo = (accessToken, callback: (Result.t(userInfoResult, string) => unit)) => 
  _userInfo(client, accessToken, jscbToResultcb(callback));

let parseHash = (url: ReasonReact.Router.url, callback: (Result.t(session, string) => unit)) => 
  _parseHash(client, { "hash": url.hash }, jscbToResultcb(
    res => Result.map(res, authResultToSession) |> callback
  )
);
